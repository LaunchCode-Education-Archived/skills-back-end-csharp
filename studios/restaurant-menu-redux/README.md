...
Within the `Restaurant` project in the `csharp-exercises` solution, begin adding the methods you designed to `Menu` and `MenuItem`. Within the `Program.cs` file, test your classes within `Main` by creating several menu items, adding them to a menu, and printing the menu to the screen.
